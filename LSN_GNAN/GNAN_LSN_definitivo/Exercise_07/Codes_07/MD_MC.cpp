...
{ 
  ...
  cout << "EQUILIBRATION PHASE: 300 steps" << endl;
  ...
  for(int iblk=1; iblk <= nblk; iblk++) //Simulation
  {
    ...
    for(int istep=1; istep <= nstep; istep++)
    {
      ...
      if(istep%10 == 0){
        //cout << "Configuration number " << nconf <<" saved." << endl;
...
//Prepare arrays for measurements
  ...
  nbins = 900;
  n_props = 6 + nbins; //Number of observables
...
//Print initial values for measured properties
  ...
  cout << "Initial virial = " << walker[iw]/(double)npart + ptail << endl;
  walker[ip] = rho*temp + (walker[iw] + (double)npart * ptail);
...
// distance i-j in pbc
      ...
      bin = 6 + int(dr/ bin_size);
      if( dr < box/2 and bin <  6 + nbins)
      {
        walker[bin] += 2;
      }
  ...
  walker[iw] = 48.0 * w; // Virial
  walker[ip] = rho * walker[it] + 1.0/(3 * vol) * walker[iw];  //Pressione
...
void Averages(int iblk) //Print results for current block
{
   double r;
   int bin;
   ofstream Epot, Ekin, Etot, Temp, Press, G, Gfin;
   const int wd=12;
    
    //cout << "Block number " << iblk << endl;
    //cout << "Acceptance rate " << accepted/attempted << endl << endl;
    
    Epot.open("/mnt/c/Users/franc/Desktop/ANNO 3/SIMULAZIONE/Esercitazioni/Esercitazione07/Risultati/Exercise07.4/MD_gas_epot.dat",ios::app);
    Ekin.open("/mnt/c/Users/franc/Desktop/ANNO 3/SIMULAZIONE/Esercitazioni/Esercitazione07/Risultati/Exercise07.4/MD_gas_ekin.dat",ios::app);
    Temp.open("/mnt/c/Users/franc/Desktop/ANNO 3/SIMULAZIONE/Esercitazioni/Esercitazione07/Risultati/Exercise07.4/MD_gas_temp.dat",ios::app);
    Etot.open("/mnt/c/Users/franc/Desktop/ANNO 3/SIMULAZIONE/Esercitazioni/Esercitazione07/Risultati/Exercise07.4/MD_gas_etot.dat",ios::app);
    Press.open("/mnt/c/Users/franc/Desktop/ANNO 3/SIMULAZIONE/Esercitazioni/Esercitazione07/Risultati/Exercise07.4/MD_gas_press.dat",ios::app);
    G.open("/mnt/c/Users/franc/Desktop/ANNO 3/SIMULAZIONE/Esercitazioni/Esercitazione07/Risultati/Exercise07.4/MD_gas_g_block.dat",ios::app);
    Gfin.open("/mnt/c/Users/franc/Desktop/ANNO 3/SIMULAZIONE/Esercitazioni/Esercitazione07/Risultati/Exercise07.4/MD_gas_g_final.dat",ios::app);
    ...
    stima_pres = rho*temp + (blk_av[iw]/blk_norm  + (double)npart * ptail)/ (3*vol);
    ...
    {
      bin = 6 + i;
      r = i * bin_size;                                    // bin_size = dr
      DeltaV = (4*M_PI/3) * ( pow(r + bin_size,3) - pow(r,3) );
      stima_g = blk_av[bin]/blk_norm;
      stima_g /= (double) (rho*npart*DeltaV);
      glob_av[bin] += stima_g;
      glob_av2[bin] += stima_g*stima_g;
      err_g = Error(glob_av[bin],glob_av2[bin],iblk);
      //stima_g = blk_av[ig + i]/blk_norm;
      //glob_av[ig + i] += stima_g;
      //glob_av2[ig + i] += stima_g*stima_g;
      //err_g = Error(glob_av[ig + i],glob_av2[ig + i],iblk);
      //g(r)
      G << setw(wd) << iblk <<  setw(wd) << r + 0.5*bin_size << setw(wd) << stima_g << setw(wd) << err_g << endl;

      if(iblk == nblk)
      {
        Gfin << r + 0.5*bin_size <<  setw(wd)  << glob_av[bin]/(double)iblk << setw(wd) << err_g << endl;
      }
    }
...
//Pressure
    Press << setw(wd) << iblk <<  setw(wd) << stima_pres << setw(wd) << glob_av[ip]/(double)iblk << setw(wd) << err_pres << endl;
...
void ConfXYZ(int nconf){ //Write configuration in .xyz format
  ...
  WriteXYZ.open("/mnt/c/Users/franc/Desktop/ANNO 3/SIMULAZIONE/Esercitazioni/Esercitazione04/Risultati/Ovito/Gas/config_" + to_string(nconf) + ".xyz");
