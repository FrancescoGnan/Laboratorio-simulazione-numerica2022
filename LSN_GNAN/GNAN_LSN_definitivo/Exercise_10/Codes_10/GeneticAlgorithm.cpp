...
bool Equal(Posizione p1, Posizione p2){

	if( p1.getX() == p2.getX() and p1.getY() == p2.getY() ) return true;
	else return false;
...
void GA::Welcome(std::vector<Posizione> new_p, int rank)
{
    ...
    {
	 mat_pop[N_pop - rank - 1][i] = New_row[i];
    }
}
//--------------------------------------
...
/*  CROSSOVER EXP
    for(unsigned int i{0}; i<N_Chrom-o; i++)
    {
        while( true )   //Trovo un elemento che non compare in copia1
        {
            int k1 = (int) rnd.Rannyu()*N_Chrom;
            int n1 = FindIndex(copia1, k1);
            if( n1 == 0 )
            {
                coda1[i] = k1;
                break;
        ...
        while( true )   //Trovo un elemento che non compare in copia2
        {
            int k2 = (int) rnd.Rannyu()*N_Chrom;
            int n2 = FindIndex(copia2, k2);
            if( n2 == 0 ) 
            {
                coda2[i] = k2;
                break;
             ...
             //NON nell'ordine in cui compaiono nel consorte
    }
    for(unsigned int i{0}; i<N_Chrom-o; i++) //Li ordino
    {
        int m1 = FindIndex(copia2, coda1[i]);
        int f1 = FindIndex(copia1, coda2[i]);
        for(unsigned int j{0}; j<N_Chrom-o-1; j++)
        {
            int m2 = FindIndex(copia2, coda1[j]);
            int f2 = FindIndex(copia1, coda2[j]);

            if( m2 < m1 )
            {
                std::swap( coda1[j] , coda1[i] );
            }
            if( f2 < f1 )
            {
                std::swap( coda2[j] , coda2[i] );
            }
        }
    }
*/
...
/*
    for(unsigned int i{0}; i<N_Chrom; i++)
    {
        while(true)
        {
            int n = FindIndex(copia1,copia2[i]);
            if( n == 0 )
            {
                coda1.push_back( copia2[i] );
                break;
            }
        }
        while(true)
        {
            int n = FindIndex(copia2,copia1[i]);
            if( n == 0 )
            {
                coda2.push_back( copia1[i] );
                break;
            }
        }
...
/////////// CROSSOVER 2
                //auto it = find( copia1.begin(), copia1.begin()+o-1, copia2[j] );
                //if( it == copia1.begin()+o-1 and copia2[j] != copia1[o-1])
/*

        int o = (int) rnd.Rannyu(0, N_Chrom - 1 );
        cout << "o = " << o << endl;
        ...
        std::vector<int> coda1{}, coda2{};

        int i{0};
            while( coda1.size() < N_Chrom - o )
            {
                int it1 = Search(copia1,copia2[i]);

                //auto it1 = find(copia1.begin(),copia1.begin() + o,copia2.at(i));  //cerco copia2[i] nella prima metà di copia1
                //if( it1 != copia1.begin()+o ) //se trova l'elemento
                if( it1 == 1 )
                {
                    coda1.push_back( copia2[i] );
                }
        ...
        int j{0};
            while( coda2.size() < N_Chrom - o )
            {
                
                int it2 = Search(copia2,copia1[j]);
                //auto it2 = find(copia2.begin(), copia2.begin() + o,copia1.at(i));
                //if( it2 != copia2.begin()+o )
                if(it2 == 1)
                {
                    coda2.push_back( copia1[j] );
                }
        ...
        //sovrascrivo in fondo i due figli sopra i due peggiori
        for(unsigned int i{0}; i<o; i++)
        {
            mat_pop[N_pop-2][i] = copia1[i];
            mat_pop[N_pop-1][i] = copia2[i];
        }
        for(unsigned int i{o}; i<N_Chrom; i++)
        {
            mat_pop[N_pop-2][i] = coda1[i];
            mat_pop[N_pop-1][i] = coda2[i];
        }
